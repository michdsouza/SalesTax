...
describe Receipt do
  it "should format results for one added product" do
    imported_card = Product.new("greeting card", 21.40, Tax.IMPORTED(Tax.OTHER))
    receipt = Receipt.new
  ...
  it "should generate line items when several products are added" do
    imported_perfume = Product.new("imported bottle of perfume", 27.99, Tax.IMPORTED(Tax.OTHER))
    perfume = Product.new("bottle of perfume", 18.99, Tax.OTHER)
    pills = Product.new("packet of headache pills", 9.75, Tax.MEDICINE)
    imported_chocolates = Product.new("box of imported chocolates", 11.25, Tax.IMPORTED(Tax.FOOD))
    receipt = Receipt.new
